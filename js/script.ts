...
let description = document.querySelector(".description") as HTMLHeadingElement;
let callToAction = document.querySelector(".cta") as HTMLHeadingElement;
let refresgBtn = document.querySelector(".button") as HTMLButtonElement;

/* - - - declare form variables - - - */
let form = document.querySelector("#dateform") as HTMLFormElement;
...
function validate(date, gender) {
  let userDate: number = Number(date.substring(8, 10));
  let userMonth: number = Number(date.substring(5, 7) - 1);
  let userYear: number = Number(date.substring(0, 4));
  let timeline: Date = new Date();
  let currentDate: number = timeline.getDate();
  let currentMonth: number = timeline.getMonth();
...
form.addEventListener("submit", (e) => {
  e.preventDefault();
  let gender: String = form.gender.value;
  ...
  let day: number = getWeekDay(date.value);
  let weekDay: String = daysOfWeek[day];
  let personality: String[] = getPersonality(gender, day);

  /* - - - give feedback to the user - - - */
  title.innerHTML = `Your Akan name is ${personality[1].replace(/^\w/, (c) =>
    c.toUpperCase()
  )}<br>( ${weekDay.replace(/^\w/, (c) => c.toUpperCase())} Born )`;
  description.innerHTML = String(personality[0]);
  title.setAttribute("style", "animation: appear 3s");
  description.setAttribute("style", "animation: appear 3s");
  callToAction.setAttribute("style", "display: none");
  form.setAttribute("style", "display: none");
  refresgBtn.setAttribute("style", "display: block; animation: appear 3s");
});
