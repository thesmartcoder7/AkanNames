...
var callToAction = document.querySelector(".cta");
var refresgBtn = document.querySelector(".button");
...
function getWeekDay(date) {
    var fullDate = new Date(Number(date.substring(0, 4)), Number(date.substring(5, 7) - 1), Number(date.substring(8, 10)));
    return fullDate.getDay();
}
/* - - - validate user input - - - */
function validate(date, gender) {
    var userDate = Number(date.substring(8, 10));
    var userMonth = Number(date.substring(5, 7) - 1);
    var userYear = Number(date.substring(0, 4));
    var timeline = new Date();
    var currentDate = timeline.getDate();
    var currentMonth = timeline.getMonth();
    var currentYear = timeline.getFullYear();
    if (date === "") {
        if (gender === "") {
...
form.addEventListener("submit", function (e) {
    ...
    var day = getWeekDay(date.value);
    var weekDay = daysOfWeek[day];
    var personality = getPersonality(gender, day);
    /* - - - give feedback to the user - - - */
    title.innerHTML = "Your Akan name is ".concat(personality[1].replace(/^\w/, function (c) {
        return c.toUpperCase();
    }), "<br>( ").concat(weekDay.replace(/^\w/, function (c) { return c.toUpperCase(); }), " Born )");
...
refresgBtn.addEventListener("click", function () {
